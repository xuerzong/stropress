...
你可以在 `docs/config.ts` 里通过 `footer` 配置页面底部内容。
...
你可以在 `docs/config.ts` 中通过 `markdown.codeTheme` 配置代码块高亮主题。
...
如果 `config.ts` 里的 `home` 配置不够灵活，可以直接在 `config.ts` 同目录放置 Astro 首页文件：
