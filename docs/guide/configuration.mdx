...
Use `docs/config.ts` to define the navigation structure.
...
Use `footer` in `docs/config.ts` to render footer text under the page content.
...
Set `site.url` in `docs/config.ts` to control canonical URLs and OG image URLs in generated HTML.
...
You can keep the default behavior or set it explicitly in `docs/config.ts`:
...
Configure code block highlighting theme via `markdown.codeTheme` in `docs/config.ts`.
...
See the full built-in theme list in Shiki: https://shiki.style/themes
...
Define `locales` in `docs/config.ts`, using path prefixes as keys:
...
If `defineConfig`-based `home` configuration is too limiting, you can add Astro homepage files next to `config.ts`:
